...
class DataCollatorForSeq2Seq:
    ...
    def __call__(self, features: List[Dict[str, Any]], return_tensors: Optional[str] = None):
        ...
        # Convert any lists in the features to tensors, except for strings
        for k, v in features.items():
            if isinstance(v, list) and all(isinstance(i, (int, float)) for i in v):
                features[k] = torch.tensor(v)
            elif isinstance(v, list):
                print(f"Skipping conversion for {k}: contains non-numerical data")
