...
import sys
import os
sys.path.append("/rds/user/lmph2/hpc-work/LLM_fusion/FuseLLM")
import torch
import argparse
import math
...
if __name__ == "__main__":
    ...
    def safe_mean(x):
        ...
        if all(isinstance(i, (int, float)) for i in x):
            ...
            return 0  # or handle differently, such as skipping these cases
    ...
    def packing_dataset(examples):
        ...
        if args.metric_level == "sequence":
            for k, v in result.items():
                if "metric_ce" not in k:
                    continue
                result[k] = [safe_mean(x) if isinstance(x, list) else x for x in v]
        else:
            remove_keys = []
            for k, v in result.items():
                ...
                result[k.replace("per_step_", "")] = [
                    safe_mean(x) if isinstance(x, list) else x for x in v
                ]
